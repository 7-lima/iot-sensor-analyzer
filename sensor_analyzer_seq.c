...
int main(int argc, char *argv[]) {
    ...
    num_linhas = carregar_arquivo(argv[1]);
    ...
    detectar_anomalias_sequencial();

    clock_gettime(CLOCK_MONOTONIC, &t_fim);
    double tempo = (t_fim.tv_sec - t_inicio.tv_sec) + (t_fim.tv_nsec - t_inicio.tv_nsec) / 1e9;

    exibir_resultados("RESULTADOS - ANALISADOR SEQUENCIAL",
                      "Estrategia: processamento sequencial em memoria",
                      0,
                      tempo);
