...
static void *thread_processar_mutex(void *arg) {
    ...
    for (long i = a->inicio; i < a->fim; i++) {
        ...
        EstatisticaSensor *s = &sensores_global[idx];
        if (!s->ativo) {
        ...
        if (strcmp(l->status, "ALERTA") == 0) total_alertas++;
        else if (strcmp(l->status, "CRITICO") == 0) total_criticos++;

        if (strcmp(l->tipo, "energia") == 0) energia_total += l->valor;
...
int main(int argc, char *argv[]) {
    ...
    executar_paralelo(num_threads, thread_processar_mutex);
    ...
    exibir_resultados("RESULTADOS - ANALISADOR PARALELO COM MUTEX",
                      "Estrategia: estado global compartilhado + mutex no hot path",
                      num_threads,
                      tempo);
